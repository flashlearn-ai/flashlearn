...
- Commands are `flashlearn init [directory]`, `flashlearn generate [directory]`, `flashlearn start [directory]`, `flashlearn review [directory]`, `flashlearn project show`, `flashlearn project status`, `flashlearn question get <card-id>`, and `flashlearn question list`.
- `review` is the CLI-owned multiple-choice TUI in `src/terminal-review.ts`: 1–4 selects an answer and automatically submits correct/incorrect, Enter/Space continues after feedback, Q/Ctrl+C/Ctrl+D quits. Shuffle up to four distinct deck answers, preferring same-topic/source distractors; do not invent answers or fall back to recall when fewer than two distinct answers exist. It requires interactive stdin/stderr, uses the same `CliService.study` review composition as the browser, and saves at most 12 reviews per session (including immediately due repeats). Only confirmed saves count; failures stop the session. Empty decks give generation guidance. Restore raw mode/listeners on exit and strip terminal control sequences from displayed source content. Scheduling and storage remain owned by their packages.
...
- Detected Copilot is first/default in the interactive inference menu; Enter opts in. Missing/noninteractive input remains offline, and unavailable Copilot defaults to heuristic.
- Generation starts with an explicit INFERENCE SOURCE stage: one Copilot/OpenAI/Claude/custom/heuristic menu, with environment endpoint precedence unless `--inference-source` overrides it. Keys are masked/current-run-only. Custom URLs require HTTP(S) without embedded credentials; optional auth headers are validated. Offline mode extracts complete definitions, explanations and code-comment responsibilities, filtering procedural/media content without inventing synthesis or categories. Evaluate with `packages/cli/scripts/evaluate-heuristic.mjs`.
...
- Do not move scheduling, extraction, persistence, or browser UI logic into the CLI. Terminal review presentation is CLI-owned.
