...
- Live study uses `GET /api/cards` for deck counts, `GET /api/cards/next` for server-selected due cards, and `GET /api/cards/:id` for explicit answer reveal. There is no live topic chooser: the locked next-card endpoint has no topic or exclusion filter. Reload starts a fresh transcript using the persisted server schedule; a next-card `404` means nothing is due, not an empty project.
...
- Live sessions are capped at 12 acknowledged reviews (`SESSION_LIMIT`); immediately due incorrect-card repeats count toward the cap. Demo and fixture/URL practice use session-only multiple choice, capped at 12 cards: selected topics take turns filling a session, the starting topic rotates when topics outnumber slots, and per-topic cursors advance by cards actually dealt. Practice ratings and cursors reset on reload.
...
- `.github/workflows/npm-release.yml` publishes on GitHub Release `published` events, not tag pushes. Tag, manifest version, prerelease flag, and main ancestry must agree. OIDC uses the `npm-publish` environment. Reruns skip only identical registry artifacts; the final job attaches the tested tarball/receipt to the existing release.
