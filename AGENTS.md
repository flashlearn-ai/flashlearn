...
- `review` is the CLI-owned keyboard TUI in `src/terminal-review.ts`: Enter/Space reveals, 1–4 submits incorrect/hard/correct/easy, Q/Ctrl+C/Ctrl+D quits. It requires interactive stdin/stderr, uses the same `CliService.study` review composition as the browser, and saves at most 12 reviews per session (including immediately due repeats). Only confirmed saves count; failures stop the session. Empty decks give generation guidance. Restore raw mode/listeners on exit and strip terminal control sequences from displayed source content. Scheduling and storage remain owned by their packages.
