...
AI generation finishes with an LLM category pass over the accepted questions and answers. Meaningful learning labels are saved in `Card.tags`, with at least five cards per category and exactly one category per card. Too few cards or invalid/failed grouping stops the run before new cards are saved. Offline and existing untagged decks retain directory grouping. Generation calls have a 32-second timeout and categorization has an 18-second timeout; scanning/storage/provider overhead still affects total duration.
