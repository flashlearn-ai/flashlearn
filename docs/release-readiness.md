...
- `flashlearn review [directory]` (or `--project`) runs the keyboard TUI with interactive stdin/stderr: Enter/Space reveals, 1–4 rates, Enter/Space continues after save, and Q/Ctrl+C/Ctrl+D quits. Verify hidden answers, source attribution, all four persisted ratings, no-due versus empty-deck messages, terminal cleanup, save-failure exit 1, and the 12-saved-review cap including immediately due repeats. Review reuses the browser's learning/storage composition without starting a server or making model calls. Terminal source text must not execute control sequences. Noninteractive review exits 1 with guidance.
