...
An existing npm version is skipped only when its registry integrity matches the freshly tested tarball exactly; differing bytes fail and require a new version. After publishing, verification waits about 15 seconds for registry replication and then rechecks the published integrity every 5 seconds for up to ten attempts, so a slow replica fails the job only after roughly a minute; differing bytes still fail immediately. A retry does not change dist-tags or add provenance to an existing manual publication. The final job uploads the tested tarball and receipt to the selected release. A GitHub Release becomes visible before npm publication completes; check the workflow result before announcing availability.
