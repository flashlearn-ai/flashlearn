...
- `flashlearn review [directory]` (or `--project`) runs the multiple-choice TUI with interactive stdin/stderr: 1–4 selects a shuffled answer and automatically saves correct/incorrect, Enter/Space continues after feedback/save, and Q/Ctrl+C/Ctrl+D quits. Verify unique deck-derived choices, same-topic/source preference, feedback/source attribution after selection, both persisted results, insufficient-answer guidance, no-due versus empty-deck messages, terminal cleanup, save-failure exit 1, and the 12-saved-review cap including immediately due repeats. Review reuses the browser's learning/storage composition without starting a server or making model calls. Terminal source text must not execute control sequences. Noninteractive review exits 1 with guidance.

- Detected Copilot appears first and is the interactive inference default (Enter accepts it). Without Copilot, Enter selects heuristic. Null/absent noninteractive input remains offline even with Copilot on PATH; explicit heuristic selection and configured endpoint precedence remain supported.

- The INFERENCE SOURCE step shows all provider choices, even when Copilot is installed. Verify `generate --inference-source openai|claude|custom|heuristic`, masked current-run-only keys, custom HTTP(S) URL/model and optional header authentication, cancellation/invalid input, and environment precedence/explicit overrides. Empty-deck start uses the same menu. Offline mode extracts complete definitions/prose/comments, omits procedure/media fragments, and is clearly labeled as non-LLM recall. Run the package-local heuristic evaluation script to compare the previous heading-based baseline and tuned output; generated content remains local.

- Generation caps new/updated cards at 100 per run and reports six numbered stages on stderr: scan, selection, generation, quality review, categories and save. Detail events show batch outcomes, active/failed/reused counts, request timing and category-repair attempts. Plain output has ten-second heartbeats and TTY status fits the terminal. Existing decks are not pruned. `generate --copilot` opts in with `auto` fast routing; `--copilot-model <name>` overrides the model. Explicit Copilot takes precedence over endpoint environment variables. Classification prioritizes relevant sources and groups code/docs into up to eight parallel four-file batches with 7,000-character excerpts. Generation and category calls allow 15 minutes each. Completed batches and categorized results are atomically checkpointed until card saves finish; matching retries resume without repeating successful inference, while changed source content invalidates stale work. `generate --fresh` starts over. Validate category-timeout, failed-batch and card-save recovery, credential omission and clean error-line formatting. Categories still require at least five cards; unfinished candidates are not study cards. Offline/legacy cards retain path grouping. No filler is added. Run `node packages/cli/scripts/benchmark-generation.mjs /path/to/repo auto --keep` after building; the one-minute metric is informational now.

- Every command defaults to cwd. `--project`/`-p` selects a directory for one invocation; relative paths resolve against cwd. Positional directories remain supported for `init`, `generate`, `start`, and `review`, mutually exclusive with `--project`.
